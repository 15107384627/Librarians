...
public class Borrow extends JFrame {
	...
	private void tf_book_idKeyTyped(KeyEvent e) {
		...
		if (e.getKeyChar() == '\n') {
			...
			BookEntity book = BookDao.selectBook(id); // 提取指定编号的图书信息
	...
	private void tf_reader_idKeyTyped(KeyEvent e) {
		...
		if (e.getKeyChar() == '\n') {
			...
			ReaderEntity reader = ReaderDao.selectReader(id); // 提取指定编号的读者信息
	...
	private void btn_borrowActionPerformed(ActionEvent e) {
		...
		String sql = "insert into borrow(book_id,reader_id,borrow_date,back_date,is_back) values('"
				...
				+ "','"
				+ back_date + "',0)";
		String sqlstr="select max_num from reader where id="+reader_id;
		ResultSet rs=BaseDao.executeQuery(sqlstr);
		// 执行数据库操作
